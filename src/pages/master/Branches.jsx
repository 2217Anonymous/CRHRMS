...
import { BRANCHSTATUS } from '../../services/api/Master'
...
import { fetchBranchData } from '../../Redux/slice/Master/Branch'
...
export default function Branches() {
    ...
    const branchList = useSelector((state) => state.branch.branchesList.Data)
    ...
    useEffect(() => {
        ...
        const getBranchList = (() => {
            if (branchList) {
                const tableData = branchList.map((res) => ({
                    NAME        : res.Name,
            ...
            <Col xl={12}>
                <Card>
                    <Card.Header>
                        ...
                        <BranchModel isOpen={isModalOpen} onClose={closeModal} />
                        {
                            checkPermission('Branches_Add') ? <button style={{float:'right'}} className='d-flex ms-auto mx-2 btn btn-outline-success' onClick={openModal}>Add Branches Group</button> :''
                        }
                    </Card.Header>
                    <Card.Body>
                        {
